...
public class VisitorInterpretator extends Visitor{
        ...
        try {
            ...
            if (!selectors.isEmpty()) {
                for (int k = 0; k < selectors.size() - 1; k++) {
                    ...
                    if (selectors.get(k) instanceof SelectorData) {
                ...
                if (selectors.get(selectors.size() - 1) instanceof SelectorData) {
        ...
        try {
            ...
            if (f != null) {
                ...
                throw new RuntimeException(
                        " (" + callCmd.getLine() + ", " + callCmd.getColumn() + ") Função não definida " + callCmd.getName());
            }
    ...
    public void visit(CallExp callExp) {
        try {
            ...
            if (f != null) {
                ...
                throw new RuntimeException(
                        " (" + callExp.getLine() + ", " + callExp.getColumn() + ") Função não existe " + callExp.getName());
            }
            ...
            if (env.peek().containsKey(lValue.getId())) {
                ...
                if (lValue.getSelectors().size() != 0) {
                    ...
                    for (Selector lv : lValue.getSelectors()) {
                        ...
                        if (lv instanceof SelectorData) {
                            obj = ((HashMap<String, Object>) obj).get((String) operands.pop());
                        } else if (lv instanceof SelectorArray) {
                ...
                throw new RuntimeException(
                        " (" + lValue.getLine() + ", " + lValue.getColumn() + ") não declarado " + lValue.getId());
            }
    ...
    public void visit(New ne) {
        try {
            ...
            if (bType instanceof TypeIDType) {

                idType = true;
                TypeIDType typeIdType = (TypeIDType) bType;
                a = new HashMap<>();
                Data data = types.get(typeIdType.getIdType());
        ...
        try {
            ...
            if (!value.getSelectors().isEmpty()) {
                ...
                for (int k = 0; k < value.getSelectors().size() - 1; k++) {
                    ...
                    if (value.getSelectors().get(k) instanceof SelectorData) {
                ...
                if (value.getSelectors().get(value.getSelectors().size() - 1) instanceof SelectorData) {
