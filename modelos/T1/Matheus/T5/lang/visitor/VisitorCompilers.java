...
public class VisitorCompilers extends Visitor {
...
//Variaveis criadas para o compilar
	private ArrayList<HashMap<String, SType>> compilersEnvs;
  private ArrayList<STypeFunc> compilersFuncs;


  public VisitorCompilers() {
    ...
    typesData = new HashMap<String, STypeData>();
		...
		compilersEnvs = new ArrayList<HashMap<String, SType>>();
  	compilersFuncs = new ArrayList<STypeFunc>();
  }
  ...
  public void visit(Prog prog) {
    ...
    if (prog.getFuncs() != null) {
      ...
      for (Func fn : prog.getFuncs()) {
        ...
        fn.accept(this);
				HashMap<String, SType> aux = new HashMap<String, SType>();
    		Set<String> keys = env.keySet();
    		for (String key : keys) {
        	aux.put(key, env.get(key));
    		}
    		compilersEnvs.add(aux);
      }
      if (mainFuncs == null) {
        throw new RuntimeException(
  ...
  public void visit(Return ret) {
    ...
    if (tempFunc.getReturnable() != null) {
      if (ret.getExpressions().length == tempFunc.getReturnable().length) {
        ...
        if (!eqReturnables) {
          throw new RuntimeException(
            ret.getLine() + ", " + ret.getColumn() + ": Tipos diferentes de retornos!"
          );
        }
      } else {
        throw new RuntimeException(
          ret.getLine() + ", " + ret.getColumn() + ": Numero diferentes de retornos!"
        );
      }
    } else {
      throw new RuntimeException(
  ...
  public ArrayList<HashMap<String, SType>> getCompilersEnvs()
  {
    return this.compilersEnvs;
  }

  public ArrayList<STypeFunc> getCompilersFuncs()
  {
    return this.compilersFuncs;
  }
