...
public class ASTVisitorConstruct extends langBaseVisitor<SuperNode> {
	...
	public SuperNode visitFunc(langParser.FuncContext ctx) { 
        ...
        Func nodeFunc = null;
        if (ctx.ID().getText() != null) {
            nodeFunc = new Func(line, column, ctx.ID().getText(), returnable, params, comand);
        } else {
            nodeFunc = new Func(line,column,ctx.IDTYPE().getText(), returnable, params ,comand);
        }
	...
	public SuperNode visitInt(langParser.IntContext ctx) { 
		int line = ctx.getStart().getLine();
        int column = ctx.getStart().getCharPositionInLine();
        Int nodeTypeInt = new Int(line,column);
        return nodeTypeInt;
	}
	
	@Override 
	public SuperNode visitChar(langParser.CharContext ctx) {     
		int line = ctx.getStart().getLine();
        int column = ctx.getStart().getCharPositionInLine();
        Char nodeTypeChar = new Char(line,column);
        return nodeTypeChar;
	}

	@Override 
	public SuperNode visitBool(langParser.BoolContext ctx) { 
		int line = ctx.getStart().getLine();
        int column = ctx.getStart().getCharPositionInLine();
        Bool nodeTypeBool = new Bool(line,column);
        return nodeTypeBool;
	}
	...
	public SuperNode visitFloat(langParser.FloatContext ctx) { 
		int line = ctx.getStart().getLine();
        int column = ctx.getStart().getCharPositionInLine();
        TFloat nodeTypeFloat = new TFloat(line,column);
        return nodeTypeFloat;
	}

	@Override 
	public SuperNode visitIDType(langParser.IDTypeContext ctx) { 
		int line = ctx.getStart().getLine();
        int column = ctx.getStart().getCharPositionInLine();
        IDType nodeTypeID = new IDType(line,column,ctx.IDTYPE().getText());
        return nodeTypeID; 
	}
	...
	public SuperNode visitModule(langParser.ModuleContext ctx) { 
        ...
        Exp left = (Exp) ctx.mexp().accept(this);
        Exp right = (Exp) ctx.sexp().accept(this);
        CModule nodeModule = new CModule(line, column, left, right);
        return nodeModule;
	}
	...
	public SuperNode visitSminus(langParser.SminusContext ctx) { 
        ...
        int column = ctx.getStart().getCharPositionInLine();
        Exp expression = (Exp) ctx.sexp().accept(this);
        SMinus nodeNeg= new SMinus(line, column, expression);
        return nodeNeg;
	}
	...
	@Override 
	public SuperNode visitSlcData(langParser.SlcDataContext ctx) { 
		int line = ctx.getStart().getLine();
        int column = ctx.getStart().getCharPositionInLine();
        Lvalue nodeSelectorData = (Lvalue) ctx.lvalue().accept(this);
        if (ctx.ID().getText() != null) {
            nodeSelectorData.add(new SlcData(line, column, ctx.ID().getText()));
        } else {
            nodeSelectorData.add(new SlcData(line, column, ctx.IDTYPE().getText()));
        }
        return nodeSelectorData; 
	}

	@Override 
	public SuperNode visitSlcArray(langParser.SlcArrayContext ctx) { 
		int line = ctx.getStart().getLine();
        int column = ctx.getStart().getCharPositionInLine();
        Lvalue nodeSelectorArray = (Lvalue) ctx.lvalue().accept(this);
        Exp expression = (Exp) ctx.exp().accept(this);
        nodeSelectorArray.add(new SlcArray(line, column, expression));
